...
runs D30 prompts when stdin is a TTY.
"""
...
def _build_final_rankings_rows(
    ...
    # Compute final_horizon + final_score; sort and assign rank
    out = []
    for t, d in by_ticker.items():
        s3 = d.get("score_at_fair_3mo") or 0.0
        s12 = d.get("score_at_fair_12mo") or 0.0
...
def main() -> int:
    ...
    if sys.stdin.isatty():
        try:
            prompt = (
                f"\nProceed to dispatch {counts[Tier.C] + counts[Tier.B]} API calls "
                f"for ${estimate.scenarios[2].total_usd:,.2f}? [y/N] "
            )
            answer = input(prompt).strip().lower() or "n"
        except EOFError:
            answer = "n"
            print("\n[stdin closed] aborting; pipe 'y' or use --non-interactive to bypass.")
        if answer != "y":
            print("Aborted.")
            return 0
    else:
        print("Non-interactive mode: skipping [y/N] gate. Use --dry-run to estimate without dispatching.")
    ...
    with sqlite3.connect(llm_scores_db) as scores_conn:
        ...
        run_id = open_run(
            ...
            gate_config=gates, batch_id=batch_id,
        )
        ...
        # Reload llm_scores rows for the response viewer
        scores_conn.row_factory = sqlite3.Row
    ...
    resp_html = outputs_dir / f"llm_responses_{quarter}.html"
    render_final_ranking_html(ranking_rows, rank_html, quarter=quarter, run_id=run_id)
    render_final_ranking_xlsx(ranking_rows, rank_xlsx, quarter=quarter, run_id=run_id)
    render_llm_responses_html(score_rows_for_viewer, resp_html, quarter=quarter, run_id=run_id)
