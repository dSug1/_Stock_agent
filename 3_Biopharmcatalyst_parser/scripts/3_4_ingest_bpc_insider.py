"""Module 4 — ingest a BPC insider supplement CSV into bpc_insider_supplement.

Run from `3_Biopharmcatalyst_parser/`:
    PYTHONPATH=src ../.venv/Scripts/python.exe scripts/3_4_ingest_bpc_insider.py [csv] [--snapshot-date YYYY-MM-DD]

If `csv` is omitted, picks the most recent *insider*.csv at the top of
_csv_source/ (archived copies under _csv_source/archive/ are ignored).
"""
...
def _default_csv() -> Path | None:
    """Most recent *insider*.csv at the top of _csv_source/.

    Narrower than `*.csv` so we never accidentally pick up the catalyst
    file that Module 1 owns.
    """
    src = PROJECT_ROOT / "_csv_source"
    if not src.exists():
        return None
    candidates = [
        p for p in src.glob("*.csv")
        if p.is_file() and "insider" in p.name.lower()
    ]
    if not candidates:
        return None
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        help="path to BPC insider CSV (default: most recent *insider*.csv in _csv_source/)",
    )
    parser.add_argument(
    ...
    if csv_path is None:
        print("error: no csv_path supplied and no *insider*.csv found in _csv_source/",
              file=sys.stderr)
        return 2
    ...
    conn = get_connection()
    try:
        stats = ingest_bpc_insider_csv(
            csv_path, args.snapshot_date, conn, dry_run=args.dry_run,
        )
    except SchemaValidationError as e:
        print(f"[3_4_ingest_bpc_insider] FAILED — {e}", file=sys.stderr)
        return 1
    finally:
        conn.close()

    print(f"[3_4_ingest_bpc_insider] {stats.status} — {csv_path.name} @ {args.snapshot_date}")
