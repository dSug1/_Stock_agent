"""Module 8 — Claude API rescue dispatcher.

Same machinery as scripts/3_7_deep_dive.py with three differences:
  1. Feed = catalyst_scores.rescued = 1 (via fetch_rescue_candidates)
  2. Prompt = M8 rescue preamble + M7 system prompt + M7 few-shots
     (different prompt_version so cache isolation works)
  3. Writeback persists claude_resolved_catalyst_date + catalyst_date_source
     + rescue_class onto each deep_dives row

Gated by the SAME mandatory [y/N] prompt as M7. Only `--yes` bypasses.

Usage (from 3_Biopharmcatalyst_parser/):
    PYTHONPATH=src ../.venv/Scripts/python.exe scripts/3_8_rescue_dispatch.py
    PYTHONPATH=src ../.venv/Scripts/python.exe scripts/3_8_rescue_dispatch.py --classes A,B
    PYTHONPATH=src ../.venv/Scripts/python.exe scripts/3_8_rescue_dispatch.py --tickers EVMN,SLN --yes
...
_LOG = logging.getLogger("3_8_rescue_dispatch")
...
def _usd_cost_per_call(
    *,
    ...
    """Copied from 3_7_deep_dive.py — same Anthropic billing math."""
...
def _prepare_per_group(
    ...
    *,
    ...
    """One payload per drug group. Packs are already rescue-augmented
    (rescue_class attached) by the caller before this point."""
    ...
    for g in drug_groups:
        ...
        # Per-ticker custom_id; same shape as M7 so debug helpers work.
        ...
        out.append({
            ...
            "rescue_class":       anchor.get("rescue_class"),
...
def main() -> int:
    ...
    parser.add_argument("--classes", default=None,
                        help="Comma-separated rescue classes to dispatch (A,B,C). Default: all.")
    ...
    print(f"[3_8_rescue_dispatch] config: {args.config}")
    print(f"[3_8_rescue_dispatch] prompt_version: {cfg.prompt_version}")
    print(f"[3_8_rescue_dispatch] model: {cfg.model}")
    ...
    if args.resume_run is not None:
        ...
        print("=" * 64)
        print(f"  RESUME mode  run_id={run_id}  batch_id={batch_id}")
        print("=" * 64)
        started = time.time()
        try:
        ...
        for r in results:
            ...
            per_ticker.append({
                ...
                "rescue_class":              entry.get("rescue_class"),
                "members":                   entry.get("members") or [],
            })
        wall = time.time() - started
        _write_results(
            results=results, per_ticker=per_ticker, run_id=run_id, mode="batch",
            cfg=cfg, wall_time_s=wall,
            biotech_db_path=args.biotech_db, fundamentals_db_path=args.fundamentals_db,
        )
        return 0
    ...
    print(f"[3_8_rescue_dispatch] live-price coverage: {n_live_ok}/{len(candidate_tickers)}")
    ...
    for cand in candidates:
        ...
        if pack is None:
            ...
            continue
        # Live-price overlay (same logic as M7).
    ...
    # Attach the per-anchor rescue_class onto the group anchor so
    # _prepare_per_group can find it. Anchor is one of the members; pick the
    # rescue_class from the matching raw_candidate.
    ...
    print(f"[3_8_rescue_dispatch] candidates: {n_candidates}  "
          f"drug-groups: {n_groups}  to dispatch: {n_dispatch}  "
          f"will populate {n_rows} catalyst row(s);  cache-hit skipped: {n_skipped}")
    if n_skipped > 0:
    ...
    if dedup_saved > 0:
        print(f"[3_8_rescue_dispatch] D23 drug-dedup saved {dedup_saved} API call(s)")

    if n_dispatch == 0:
        print("[3_8_rescue_dispatch] nothing to dispatch (all cached); exiting.")
        return 0
    ...
    print()
    print(f"[3_8_rescue_dispatch] estimated cost ({mode_label}): ${est_total:.2f}")
    print(f"[3_8_rescue_dispatch] cost ceiling: ${cfg.cost_ceiling_usd:.2f}")
    ...
    if args.yes:
        print(f"[3_8_rescue_dispatch] --yes specified: dispatching {n_dispatch} call(s) "
              f"for ~${est_total:.2f} without confirmation.")
    else:
        try:
            prompt = (
                f"\nProceed to dispatch {n_dispatch} RESCUE Anthropic call(s) "
            ...
            "members": [
                {
                    ...
                    "rescue_class":       m.get("rescue_class"),
    ...
    gate_config = {
        "dispatch_kind":    "m8_rescue",
        "tickers_explicit": explicit or "all rescued",
    ...
    if mode == "sync":
        with deep_dives_connect() as dd_conn:
            ...
            dd_conn.commit()
        print(f"[3_8_rescue_dispatch] dispatching {n_dispatch} via sync (run_id={run_id})...")
        ...
        print(f"[3_8_rescue_dispatch] submitting batch ({n_dispatch} request(s))...")
        ...
        print(f"  -> recover with: python scripts/3_8_rescue_dispatch.py --resume-run {run_id}")
        results = poll_and_collect_batch(
            api_key=api_key, batch_id=batch_id,
            poll_interval_s=cfg.dispatch.poll_interval_s,
            timeout_s=cfg.dispatch.timeout_s,
    ...
    _write_results(
        results=results, per_ticker=per_ticker, run_id=run_id, mode=mode, cfg=cfg,
        wall_time_s=wall,
        biotech_db_path=args.biotech_db, fundamentals_db_path=args.fundamentals_db,
    )
    return 0
...
def _write_results(
    *,
    ...
    """Parse + persist each DispatchResult. Adds D35 fields onto each row."""
    ...
    try:
        ...
        for res in results:
            ...
            rescue_class = (prep or {}).get("rescue_class")
            ...
            for member in members:
                # D35 — use Claude's resolved date for weeks-to-catalyst math
                # when available; otherwise fall back to BPC's catalyst_date_iso.
    ...
    print()
    print("-" * 70)
    print(f"Module 8 RESCUE run complete: run_id={run_id}")
    ...
    print(f"  USD if no caching:    ${list_price_total:,.2f}")
    print("-" * 70)
    if rate_limited:
        print()
